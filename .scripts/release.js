...
async function main() {
  ...
  await runIfNotDry('git', ['push', `refs/tags/v${targetVersion}`]);
  await runIfNotDry('git', ['push', 'main']);
...
function updatePackageDeps(pkg, depType, version) {
  ...
  Object.keys(deps).forEach((dep) => {
    if (dep.startsWith('@svelteness') && packages.includes(dep.replace(/^@svelteness\//, ''))) {
