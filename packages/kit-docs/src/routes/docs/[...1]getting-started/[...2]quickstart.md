...
1. Check for a `sidebar_title` frontmatter property.
2. Check for a `title` frontmatter property.
3. Try to extract a heading `# Heading`.
4. Map the filename from kebab-case to title-case (e.g., `my-file` -> `My File` ).
...
export const get = createSidebarRequestHandler({
  // This is the default.
  formatCategoryName: (dirname) => kebabToTitleCase(dirname),
});
```
